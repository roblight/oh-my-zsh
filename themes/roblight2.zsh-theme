...
function theme_precmd {
    ...
    if [[ "$promptsize + $rubypromptsize + $pwdsize" -gt $TERMWIDTH ]]; then
      ...
      PR_FILLBAR_FULL="\${(l.(($TERMWIDTH - 2))..${PR_HBAR}.)}"
    fi
...
$return_code
$PR_CYAN$PR_SHIFT_IN$PR_HBAR$PR_HBAR${(e)PR_FILLBAR_FULL}$PR_HBAR$PR_SHIFT_OUT
%{$PR_BLUE%}[$PR_MAGENTA%t%{$reset_color%}`git_prompt_info`$PR_BLUE]$PR_RED%(!.%SROOT%s.%n)$PR_GREY@$PR_YELLOW%m
$PR_GREEN%/%<<
;$PR_NO_COLOUR '

    PROMPT_WITH_GIT_PROMPT_STATUS='$PR_SET_CHARSET$PR_STITLE${(e)PR_TITLEBAR}\
$return_code
$PR_CYAN$PR_SHIFT_IN$PR_HBAR$PR_HBAR${(e)PR_FILLBAR_FULL}$PR_HBAR$PR_SHIFT_OUT
%{$PR_BLUE%}[$PR_MAGENTA%t%{$reset_color%}`git_prompt_info``git_prompt_status`$PR_BLUE]$PR_RED%(!.%SROOT%s.%n)$PR_GREY@$PR_YELLOW%m
$PR_GREEN%/%<<
;$PR_NO_COLOUR '
